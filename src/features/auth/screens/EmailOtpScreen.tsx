...
import { useResendCustomerOtp, useVerifyCustomerEmail } from '../hooks/useCustomerAuth';
import { useResendRestaurantOtp, useVerifyRestaurantEmail } from '../hooks/useRestaurantAuth';
...
type otpProps = NativeStackScreenProps<RootStackParamList, 'emailOtpVerify'>;

export default function EmailOtpScreen({ navigation, route }: otpProps) {
    ...
    //Query calls
    const { mutate: verifyCustomer } = useVerifyCustomerEmail();
    const { mutate: verfiyRestaurant } = useVerifyRestaurantEmail();

    const { mutate: resendOtpCustomer } = useResendCustomerOtp();
    const { mutate: resendOtpRestaurant } = useResendRestaurantOtp();
        ...
        if (role === 'customer') {
            verifyCustomer(data, {
                onSuccess: data => {
                    ...
                    alert.success('Verified', data?.message || 'Email verfied successfully');
    ...
    const handleResend = () => {
        ...
        if (role === 'customer') {
            resendOtpCustomer(email, {
                onSuccess: data => {
                    alert.success('Resend', data?.message || 'Otp resend successfully');
                },
                onError: (error: ApiError) => {
        ...
        >
            ...
            <View style={styles.content}>
                ...
                <Text style={styles.subtitle}>
                    ...
                    <Text style={styles.phoneHighlight}>Email {email}</Text>{' '}
