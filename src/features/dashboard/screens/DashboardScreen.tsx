...
export default function DashboardScreen({ navigation }: homeProps) {
    ...
    // ── Seed from/to + selectedRouteId once routes arrive ─────────────────────
    useEffect(() => {
        if (routes.length > 0 && !selectedRouteId) {
            const first = routes[0];
            if (!first?._id) return;
            setSelectedRouteId(first._id);
            setFrom(first.fromCity);
            setTo(first.toCity);
        }
    }, [routes]);

    // ── Derive the active route and use its ID for the restaurants query ───────
    const selectedRoute = routes.find(r => r._id === selectedRouteId) ?? routes[0];
    const routeId = selectedRoute?._id ?? '';

    // ── Fetch restaurants only when we have a real routeId ────────────────────
    ...
    // ── Swap: flip display text + switch to reverse route if it exists ─────────
    const handleSwap = () => {
        const prevFrom = from;
        const prevTo = to;
        setFrom(prevTo);
        ...
        const reverse = routes.find(r => r.fromCity === prevTo && r.toCity === prevFrom);
        if (reverse) {
            if (reverse?._id) setSelectedRouteId(reverse._id);
        }
    };
        ...
        return (
            ...
            >
                <View style={styles.restImageArea}>
                    <View
                        style={[styles.restImageBg, !isAhead && { backgroundColor: Colors.passed }]}
                    >
            ...
            >
                            ...
                            <View>
                                ...
                                <View style={styles.heroLocRow}>
                                    ...
                                    <Text style={styles.heroLocText}>
                                        {selectedRoute?.name ?? 'Sehore Highway'}
                                    </Text>
                                </View>
                            </View>
                        </View>
                ...
                <View style={styles.body}>
                    ...
                    <View style={styles.plannerCard}>
                        ...
                        <TouchableOpacity
                            style={[styles.findBtn, !routeId && styles.findBtnDisabled]}
                            onPress={() =>
                                routeId && navigation.navigate('restaurants', { routeId })
                            }
                            activeOpacity={0.88}
                            disabled={!routeId}
                        >
                            ...
                            {DELIVERY_TIMES.map(t => (
                                ...
                                >
                                    ...
                                    <Text
                                        style={[
                                            styles.timeCardText,
                                            selectedTime === t.id && styles.timeCardTextActive,
                                        ]}
                                    >
                                        {t.label}
                                    </Text>
                                    {selectedTime === t.id && (
                                        <View style={styles.timeCardActiveDot} />
                                    )}
                                </TouchableOpacity>
                            ))}
                        </View>
                            ...
                            {CATEGORIES.map(c => (
                                ...
                                >
                                    ...
                                    <Text
                                        style={[
                                            styles.catCardLabel,
                                            activeCat === c.id && styles.catCardLabelActive,
                                        ]}
                                    >
                                        {c.label}
                                    </Text>
...
const styles = StyleSheet.create({
    ...
    routeMetaText: { fontSize: 12, color: Colors.textMuted, fontWeight: '600', flexShrink: 1 },
    routeMetaDot: { width: 3, height: 3, borderRadius: 1.5, backgroundColor: Colors.border },
